...
import {endRefreshing, requestRecord, loadMoreRecord, changeStartDate, changeEndDate, emptyDate} from '../../redux/actions/profit_detail';
...
class ProfitDetail extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      datePicker: false,
        ...
        <View style={{paddingVertical: common.margin10}}>
          <View style={styles.item_container}>
            <View style={{flex: 1, flexDirection: 'row', alignItems: 'center'}}>
              <Text style={styles.item_container_text}>{`${transfer(this.props.language, 'profit01')}：${common.covertDate(
                new Date(item.settleAt),
              )}`}</Text>
              <Text style={styles.item_container_text}>{`${transfer(this.props.language, 'profit03')}：${common.removeInvalidZero(
                BigNumber(item.profit).toFixed(8, 1),
              )}`}</Text>
            </View>
            {this.props.busi_bonus == 1 || BigNumber(item.b_bonus).gt(0) ? (
              <View style={{flex: 1, flexDirection: 'row', paddingTop: common.margin10}}>
                <Text style={styles.item_container_text}>{`${transfer(this.props.language, 'profit02')}：${common.removeInvalidZero(
                  BigNumber(item.b_bonus).toFixed(8, 1),
                )}`}</Text>
                <Text style={styles.item_container_text}>{`${transfer(this.props.language, 'profit04')}：${common.removeInvalidZero(
              ...
              <View style={{flex: 1, flexDirection: 'row', paddingTop: common.margin10}}>
                ...
                <Text style={styles.item_container_text}>{`${transfer(this.props.language, 'local_18')}：${common.removeInvalidZero(
